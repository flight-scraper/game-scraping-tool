...
const getGamesAndScores = async(gameTitle) => {
  return scrapesMultiplePages(gameTitle, 5)
...
getGamesAndScores(gameName)
  ...
  .then(result => result.filter(game => game.gameTitle !== gameName).slice(0, Math.min(30, result.length)))
  .then(result => result[Math.floor(Math.random() *  Math.min(30, result.length))].gameTitle)
