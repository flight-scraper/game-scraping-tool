...
let gameName = 'final-fantasy-vii-remake';
...
getGamesAndScores(gameName)
  ...
  .then(result => result.filter(game => game.gameTitle !== gameName).slice(0, 10 || result.length))
  .then(result => result[Math.floor(Math.random() * 10 || result.length)].gameTitle)
