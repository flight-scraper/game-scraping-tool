...
// let gameName = 'the last of us part ii';
...
const scrapesMultiplePages = (gameTitle, pagesAmount) => { 
  return Promise.all([...Array(pagesAmount)].map((_, i) => {
    return scrapeUsers(gameTitle, i);
  }))
    .then(result => result.flat());   
};


const scrapeUsers = (gameTitle, page) => {
  return request
    .get(`https://www.metacritic.com/game/playstation-4/${gameTitle}/user-reviews?dist=positive&page=${page}`)
    .use(throttle.plugin())
    .retry(3)
    .then(siteData => {
      let $ = cheerio.load(siteData.text);
...
const getGamesAndScores = async(gameTitle) => {
  return scrapesMultiplePages(gameTitle, 1)
...
function finalResult(gameName) {
  const formattedName = gameName.replace(/ /g, '-').toLowerCase();
  return getGamesAndScores(formattedName)
    .then(result => result.flat())
    ...
    .then(result => result.filter(game => game.gameTitle !== gameName).slice(0, Math.min(20, result.length)))
