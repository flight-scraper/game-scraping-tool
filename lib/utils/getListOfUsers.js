...
const getGamesAndScores = async(gameTitle) => {
  let title = gameTitle.replace(/ /g, '-');
  console.log(title);
  return scrapeUsers(title)
...
const gameName = 'the last of us part ii';
getGamesAndScores(gameName)
  .then(result => result.flat())
  .then(result => result.reduce((acc, item) => {
    let addGame = true;
    acc.forEach(game => {
      if(game.gameTitle === item.reviewTitle.split('/')[3]){
        addGame = false;
        game.count++;
      }
    });
    if(addGame) {
      acc.push({
        gameTitle: item.reviewTitle.split('/')[3],
        count: 1
      });
    }
    return acc;
