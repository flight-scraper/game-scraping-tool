...
const T = new Twit({
  consumer_key: process.env.consumer_key || 2,
  consumer_secret: process.env.consumer_secret || 2,
  access_token: process.env.access_token || 2,
  access_token_secret: process.env.access_token_secret || 2
});
...
stream.on('tweet', tweet => {
  ...
  finalResult(gameName)
    ...
    .catch((err) => {
      console.log(err);
      T.post('statuses/update', { status: `@${tweet.user.screen_name}, we couldn't find anything! Make sure your spelling is correct and try again later!` });
    });
});
